	...
	protected static KeyValue outcome(RestTemplateObservationContext context) {
		try {
			if (context.getResponse() != null) {
				RestTemplateObservation.Outcome outcome = RestTemplateObservation.Outcome.forStatus(context.getResponse().getStatusCode().value());
				return KeyValue.of(RestTemplateObservation.LowCardinalityKeyNames.OUTCOME, outcome.name());
			}
		}
		catch (IOException ex) {
			// Continue
		}
		return RestTemplateObservation.Outcome.UNKNOWN.asKeyValue();
	}
