...
public class DefaultWebClientObservationConvention implements WebClientObservationConvention {
	...
	protected static KeyValue outcome(WebClientObservationContext context) {
		if (context.getResponse() != null) {
			WebClientObservation.Outcome outcome = WebClientObservation.Outcome.forStatus(context.getResponse().statusCode().value());
			return KeyValue.of(WebClientObservation.LowCardinalityKeyNames.OUTCOME, outcome.name());
		}
		return WebClientObservation.Outcome.UNKNOWN.asKeyValue();
	}
